defmodule AOC2021.DAY10 do
  ...
  def run() do
    ...
    syntax_error_score = corrupt_lines |> Enum.reduce(0, &(&2 + points(&1)))
  ...
  defp find_chunks(["}" | t], ["{" | tt]), do: find_chunks(t, tt)
  defp find_chunks([">" | t], ["<" | tt]), do: find_chunks(t, tt)
  defp find_chunks([f | _], [e | _]), do: {:error, %{expecting: Map.get(@closer, e), found: f}}
  ...
  defp close_line([h | t], acc), do: close_line(t, acc ++ [Map.get(@closer, h)])
  defp close_line([], acc), do: acc
